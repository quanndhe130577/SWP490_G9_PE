...
namespace TnR_SS.UnitTest
{
    public class TnR_SSSupervisorPurchaseUnitTest
    {
        ...
        {
            IMapper mapper = mapper = new Mapper(new MapperConfiguration(cfg => cfg.AddProfile(new MapperProfile())));
            Mock<IUnitOfWork> umock = new Mock<IUnitOfWork>();
            MockSetup(umock);
            TnR_SSSupervisor supervisor = new TnR_SSSupervisor(mapper, umock.Object);
            List<PurchaseResModel> rs = await supervisor.GetAllPurchaseAsync(traderid);
            if (traderid == 1)
            {
                Assert.NotEmpty(rs);
            }
        ...
        public async Task TestGetPurchaseByIdAsync(int pid, int uid)
        {
            ...
            if (pid == 1)
            {
                if (uid == 1)
                {
                    PurchaseResModel purchase = await supervisor.GetPurchaseByIdAsync(pid, uid);
                    Assert.NotNull(purchase);
                }
                else
                {
                    Exception ex = await Assert.ThrowsAsync<Exception>(async () => await supervisor.GetPurchaseByIdAsync(pid, uid));
        ...
        public async Task TestCreatePurchaseAsync(int poid, int traderid)
        {
            ...
            if (poid == 1)
            {
                Exception ex = await Assert.ThrowsAsync<Exception>(async () => await supervisor.CreatePurchaseAsync(purchase));
            ...
            {
                var rs = await supervisor.CreatePurchaseAsync(purchase);
        ...
        public async Task TestUpdatePurchaseAsync(int id, double commission, double sentMoney, int poid, int traderid)
        {
            ...
            if (id == 1000)
            {
                Exception ex = await Assert.ThrowsAsync<Exception>(async () => await supervisor.UpdatePurchaseAsync(update, traderid));
                Assert.Equal("Đơn mua không tồn tại", ex.Message);
            }
            else if (poid == 1000)
            {
        ...
        [InlineData(2)]
        [InlineData(1000)]
        public async Task GetAllPurchaseDetailAsync(int id)
        {
            IMapper mapper = mapper = new Mapper(new MapperConfiguration(cfg => cfg.AddProfile(new MapperProfile())));
            Mock<IUnitOfWork> umock = new Mock<IUnitOfWork>();
            MockSetup(umock);
            ...
            if (id == 1)
            {
                List<PurchaseDetailResModel> purchases = await supervisor.GetAllPurchaseDetailAsync(id);
            ...
            {
                await Assert.ThrowsAnyAsync<NullReferenceException>(async () => await supervisor.GetAllPurchaseDetailAsync(id));
            }
        }
        void MockSetup(Mock<IUnitOfWork> umock)
        {
            umock.Setup(u => u.Purchases.GetAll(It.IsAny<Expression<Func<Purchase, bool>>>(),
            ...
            umock.Setup(u => u.PurchaseDetails.GetAll(It.IsAny<Expression<Func<PurchaseDetail, bool>>>(),
            ...
            umock.Setup(u => u.ClosePurchaseDetails.GetAllByPurchase(It.IsAny<Purchase>()))
                .Returns((Purchase purchase) => closePurchaseDetails.Where(pd => pd.PurchaseId == purchase.ID).ToList());
            umock.Setup(u => u.Drums.GetDrumsByPurchaseDetail(It.IsAny<PurchaseDetail>())).Returns(drums);
            umock.Setup(u => u.Drums.GetDrumsByClosePurchaseDetail(It.IsAny<ClosePurchaseDetail>())).Returns(drums);

            foreach (Purchase purchase in purchases)
            {
                umock.Setup(m => m.Purchases.FindAsync(It.Is<int>(id => id == purchase.ID))).ReturnsAsync(purchase);
            }
            foreach (PondOwner pondOwner in pondOwners)
            {
                umock.Setup(m => m.PondOwners.FindAsync(It.Is<int>(id => id == pondOwner.ID))).ReturnsAsync(pondOwner);
            }
            foreach (FishType fishType in fishTypes)
            {
                umock.Setup(m => m.FishTypes.FindAsync(It.Is<int>(id => id == fishType.ID))).ReturnsAsync(fishType);
            }
            foreach (Basket basket in baskets)
            {
                umock.Setup(m => m.Baskets.FindAsync(It.Is<int>(id => id == basket.ID))).ReturnsAsync(basket);
            }
            foreach (Truck truck in trucks)
            {
                umock.Setup(m => m.Trucks.FindAsync(It.Is<int>(id => id == truck.ID))).ReturnsAsync(truck);
            }
            umock.Setup(m => m.UserInfors.FindAsync(It.Is<int>(id => id == 1))).ReturnsAsync(new UserInfor() { Id = 1 });
            umock.Setup(m => m.UserInfors.FindAsync(It.Is<int>(id => id == 2))).ReturnsAsync(new UserInfor() { Id = 2 });
            umock.Setup(m => m.UserInfors.GetRolesAsync(It.Is<int>(id => id == 1))).ReturnsAsync(new List<string>() { "Trader" });
            umock.Setup(m => m.UserInfors.GetRolesAsync(It.Is<int>(id => id == 2))).ReturnsAsync(new List<string>() { "WeightRecorder" });
        }
        void Setup()
        {
            date = DateTime.Now;
